...
if [ ! -e "/workspace/ComfyUI/main.py" ]; then
    ...
    uv pip install git+https://github.com/myshell-ai/MeloTTS.git 2>&1 | tee -a /workspace/logs/comfyui.log
    python -m unidic download 2>&1 | tee -a /workspace/logs/comfyui.log
    ...
    git clone --depth=1 https://github.com/AIFSH/ComfyUI-XTTS.git 2>&1 | tee -a /workspace/logs/comfyui.log && du -sh ComfyUI-XTTS | tee -a /workspace/logs/comfyui.log
    git clone --depth=1 https://github.com/hay86/ComfyUI_OpenVoice.git 2>&1 | tee -a /workspace/logs/comfyui.log && du -sh ComfyUI_OpenVoice | tee -a /workspace/logs/comfyui.log
    git clone --depth=1 https://github.com/ShmuelRonen/ComfyUI_wav2lip.git 2>&1 | tee -a /workspace/logs/comfyui.log && du -sh ComfyUI_wav2lip | tee -a /workspace/logs/comfyui.log


    # 3) Проверка весов Wav2Lip (опциональная авто-загрузка)
    W2L_CKPT="./ComfyUI_wav2lip/Wav2Lip/checkpoints/wav2lip_gan.pth"
    WAV2LIP_MODEL_URL="https://huggingface.co/Nekochu/Wav2Lip/resolve/main/wav2lip_gan.pth?download=true"
    if [ ! -f "$W2L_CKPT" ] && [ -n "${WAV2LIP_MODEL_URL:-}" ]; then
      echo "[start] downloading Wav2Lip checkpoint..."
      mkdir -p "$(dirname "$W2L_CKPT")"
      curl -L --retry 3 --connect-timeout 10 "$WAV2LIP_MODEL_URL" -o "$W2L_CKPT" || true
    fi
